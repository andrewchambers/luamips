...
echo "" | timeout 2s qemu-system-mips -machine mips  -cpu 4kc -kernel kern -nographic -serial file:$DIR/hash1.out
grep "checksum =" $DIR/hash1.out 

set +e

#this is gonna be alot slower.
timeout 60s luajit $DIR/../../emu.lua ./kern.srec > $DIR/hash2.out
